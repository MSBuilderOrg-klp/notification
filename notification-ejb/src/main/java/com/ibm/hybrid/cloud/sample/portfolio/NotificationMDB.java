 ...
 */
@MessageDriven(name = "NotificationMDB", mappedName = "jms/Portfolio/NotificationQueue",
	activationConfig = {
...
public class NotificationMDB implements MessageListener {
	...
	public NotificationMDB() {
		super();
		System.out.println("Initializing NotificationMDB");
		...
		try {
			...
			if (url == null) {
				...
				System.out.println("NotificationMDB - Initialization completed successfully!");
			}
		} catch (Throwable t) {
			t.printStackTrace();
		}
	...
	public void onMessage(Message message) {
		...
		System.out.println("NotificationMDB onMessage() invoked");
		if (message instanceof TextMessage) try {
			TextMessage text = (TextMessage) message;
			String json = text.getText();

			System.out.println("NotificationMDB - Sending "+json+" to "+url);
	...
	private static JsonObject invokeREST(String verb, String uri, String input, String user, String password) throws IOException {
		...
		if (input != null) {
			System.out.println("NotificationMDB:invokeREST() writing JSON outputstream");
